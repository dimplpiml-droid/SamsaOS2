namespace SamsaOS.GUI
{
    internal class PerformanceWatchdog : Process
    {
        ...
        {
            if (SamsaOS.Sysinfo.InstalledRAM < 120) { Notify("Less than 128 MB RAM detected; blur effects disabled. System stability may be impacted.", Icons.warn); }
            else if (SamsaOS.Sysinfo.InstalledRAM < 250) { Notify("Less than 256 MB RAM detected; System stability may be impacted.", Icons.info); }
            return 0;
        }
        internal override void Update()
        {
            if ((int)(SamsaOS.Sysinfo.UsedRAM / SamsaOS.Sysinfo.InstalledRAM * 100) > 79)
            {
                if (!acknowledged1)
                {
                ...
                if ((int)(SamsaOS.Sysinfo.UsedRAM / SamsaOS.Sysinfo.InstalledRAM * 100) > 89)
                {
                    if (!acknowledged2)
                    {
                    ...
                    if ((int)(SamsaOS.Sysinfo.UsedRAM / SamsaOS.Sysinfo.InstalledRAM * 100) > 94)
                    {
                        if (!acknowledged3)
                        {
