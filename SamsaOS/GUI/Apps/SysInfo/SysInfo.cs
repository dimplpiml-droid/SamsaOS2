...
namespace SamsaOS.GUI
{
    internal class InfoSystem: Window
    {
        ...
        {
            Background(GUI.DarkGrayPen.ValueARGB);
            DrawString(SamsaOS.Sysinfo.Main(), Color.White.ToArgb(), GUI.DarkGrayPen.ValueARGB, 10, 10);
            DrawString("FPS: " + GUI.fps + " | Frametime: " + (1000.0f / GUI.fps).ToString("0.0") + " ms", Color.White.ToArgb(), GUI.DarkGrayPen.ValueARGB, 10, 90);
            DrawString(SamsaOS.Sysinfo.Ram(), Color.White.ToArgb(), GUI.DarkGrayPen.ValueARGB, 10, 110);
        }
    }
...
namespace SamsaOS.Commands
{
    internal class SystemInfo : CommandsTree
    {
        ...
        internal override int Execute(string[] args, string rawInput, CommandShell shell)
        {
            ...
            {
                shell.Print = SamsaOS.Sysinfo.Main() + "\n" + SamsaOS.Sysinfo.Ram();
                return 0;
            }
            return 1;
        }
