...
namespace SamsaOS.GUI
{
    internal class Menu : Process
    {
        ...
        internal override void Update()
        {
            ...
            GUI.canvas.DrawString(SamsaOS.Date.CurrentTime(true), GUI.font, GUI.WhitePen, (int)GUI.ScreenX - 70, (int)GUI.ScreenY - 20);
            ...
            if (MouseManager.Y > GUI.ScreenY - 30)
            {
                ...
                else if (MouseManager.X > (int)GUI.ScreenX - 75)
                {
                    ...
                    else { GUI.canvas.DrawString(SamsaOS.Date.CurrentDate(true, false), GUI.font, GUI.WhitePen, (int)GUI.ScreenX - 200, (int)GUI.ScreenY - 45); }
                }
                else if (Kernel.networkConnected && MouseManager.X > (int)GUI.ScreenX - 100)
                {
                    if (!Opened) { GUI.canvas.DrawImageAlpha(start, 5, (int)GUI.ScreenY - 27); }
                    if (GUI.Pressed) { }
                    else { GUI.canvas.DrawString(SamsaOS.Net.GetInfo(), GUI.font, GUI.WhitePen, (int)GUI.ScreenX - 250, (int)GUI.ScreenY - 45); }
                }
                else
                {
                    if (!Opened) { GUI.canvas.DrawImageAlpha(start, 5, (int)GUI.ScreenY - 27); }
