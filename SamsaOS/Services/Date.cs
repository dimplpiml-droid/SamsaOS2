...
namespace SamsaOS.GUI
{
    internal class Date : Window
    {
        ...
        internal override int Start()
        {
            ...
            DrawString(SamsaOS.Date.CurrentTime(true), Color.LimeGreen.ToArgb(), Color.Black.ToArgb(), x - (x / 2) - 35, 10);
            DrawString(SamsaOS.Date.CurrentDate(true, true), Color.White.ToArgb(), Color.Black.ToArgb(), x - (x / 2) - 100, 25);
            return 0;
        }
    ...
    internal class Date : CommandsTree
    {
        ...
        internal override int Execute(string[] args, string rawInput, CommandShell shell)
        {
            if (args[0] == "time" || args[0] == "date")
            {
                ...
                string str = "";
                if (!noDate) { str += SamsaOS.Date.CurrentDate(true, true) + " "; }
                if (!noTime) { str += SamsaOS.Date.CurrentTime(true); }
                shell.Print = str;
                return 0;
            }
            return 1;
